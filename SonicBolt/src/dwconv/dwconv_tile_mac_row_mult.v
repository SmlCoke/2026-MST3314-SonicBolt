        ...
        for (g_ch = 0; g_ch < 4; g_ch = g_ch + 1) begin : CH_LOOP
            for (g_oy = 0; g_oy < 2; g_oy = g_oy + 1) begin : OY_LOOP
                for (g_ox = 0; g_ox < 2; g_ox = g_ox + 1) begin : OX_LOOP
                    // 乘法节点
                    wire signed [15:0] p_0 = $signed(row_window_data_reg[(g_ch * 8 + g_oy * 4 + g_ox + 0) * 8 +: 8]) * $signed(weight_row_data_reg[(g_ch * 24) + (0 * 8) +: 8]);
                    wire signed [15:0] p_1 = $signed(row_window_data_reg[(g_ch * 8 + g_oy * 4 + g_ox + 1) * 8 +: 8]) * $signed(weight_row_data_reg[(g_ch * 24) + (1 * 8) +: 8]);
                    wire signed [15:0] p_2 = $signed(row_window_data_reg[(g_ch * 8 + g_oy * 4 + g_ox + 2) * 8 +: 8]) * $signed(weight_row_data_reg[(g_ch * 24) + (2 * 8) +: 8]);
