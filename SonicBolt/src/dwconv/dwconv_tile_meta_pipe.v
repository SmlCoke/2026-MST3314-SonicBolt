`timescale 1ns / 1ps
/*
 * 模块名称: dwconv_tile_meta_pipe
 * 作者: SonicBolt 团队
 * 日期: 2026-03-29
 * 版本: v1.0
 *
 * 功能概述:
 *   为 dwconv_tile 的各级流水统一传递 metadata。
 *   metadata 的意思是元数据，指的是与数据流相关但不直接参与算术的控制信息。
 *   目前元数据包括：
 *     - valid：当前数据是否有效
 *     - last：当前数据是否为整张图的最后一个 token
 *     - pos：当前 token 的 pos 编号，范围 0~8
 *     - group：当前 token 的 group 编号，范围 0~7
 *     - fire：第三层的启动信号
 *
 * 设计说明:
 *   - 本模块只负责打一拍寄存，不参与任何算术。
...
module dwconv_tile_meta_pipe (
    ...
    output reg        out_fire   // 打一拍后的第三层启动信号
);
