...
> 当前系统所属版本：SonicBolt: Post Process v1.4（FC + Sigmoid 全链路只读无写口）
...
2. FC 以其带有的 `pos\group` 进行寻址，从 `72×64` 只读 ROM 中提取权重与偏置，直接更新帧级累加器（FC Frame Accum）；FC 参数链路全程无写口。
3. 随着一张图流转完成（即 `last = 1` 触发），完成帧级量化的缩放处理后，交由硬件 Sigmoid LUT 转换为 0～255 的查找区间直接输出 FP32 结果。Sigmoid LUT 由 ROM 初始化文件固化，运行时不支持写入。 
...
4. `fc.v`：**全连接网子系统顶层**，用于整合其附属组件：
    - `fc_param_store_rom.v`：固化 FC 权重与偏置参数，仅保留读口；通过 `pos/group` 完成寻址，并保持“读请求后一拍输出寄存器更新”的时序语义。
...
5. `sigmoid.v`：**只读 LUT 映射模块**，通过预置查表实现概率激活换算。LUT 内容由 `src/post_process/rom/sigmoid_lut_rom.mem` 初始化，运行时仅保留读链路。

### 3.2 架构特点——Sigmoid 表
在后处理的末端，由于硬件无法处理复杂的自然指数 $1/(1+e^{-x})$。系统采用一个 **256 项、每项 32bit（FP32）** 的查找表进行直接映射。通过将**原输入值作为寻址器直接抓取对应 FP32 值充当分类结果**。

当前版本中，Sigmoid LUT 已切换为 ROM 固化：
- ROM 初始化文件路径为 `src/post_process/rom/sigmoid_lut_rom.mem`。
- 数据准备脚本 `data/prepare_data_common.py` 在生成 `prepared_test/sigmoid_lut/lut_words.mem` 时，会同步写入上述 ROM 文件。
- `post_process_subsystem -> cnn -> cnn_chip -> TB` 全链路已删除 `sigmoid_lut_wr_*` 写口，不再支持运行时参数写入。

### 3.3 时序说明与修正建议
- FC 参数链路为 ROM 固化只读，系统不再支持 FC 运行时写入（全链路只读无写口）。
- Sigmoid 参数链路同样为 ROM 固化只读，系统不再支持 Sigmoid LUT 运行时写入（全链路只读无写口）。
- Sigmoid 保持原 `RD_IDLE/RD_WAIT0/RD_WAIT1` 三态读流程，维持“地址发起后一拍取数”的时序语义，不压缩现有延迟。
